...
Provenance tags: `[BG]` Black Gate (Ultima VII original), `[SI]` Serpent Isle, `[U4]` Ultima IV (Virtues canon), `[BR]` Britannia Reborn (new layer). Most of this document is `[BR]` — Ultima VII's quests were hand-authored single-file scripts; nothing in the original used templated parameter slots or runtime binding.
