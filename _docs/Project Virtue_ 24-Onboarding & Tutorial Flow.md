...
Status: Living Design Reference — Normative spec for first-launch flow, Highmere Town Opening, Tutorial Shard, UGC editor unlock, persistent-shard gating, and re-onboarding
...
## 3. Highmere Town Opening

The BG-equivalent diegetic intro `[BG]`. No HUD overlays. No tutorial popups. The game is the tutorial.

### 3.1 Cinematic Stub

Red moongate opens in the Avatar's bedroom (the meta-fictional Real-World framing, faithful to BG `[BG]`). Lord Avermere's voice summons the Avatar through. Transition to Highmere main square. In Phase 1, this is a **text intro screen** (§14); the full cinematic is Phase 2.
...
| Designed-for-failure | Yes — players may complete the four milestones in any order, on any path through Highmere. The game does not advance them; the world is open. |
...
A small bordered garden outside Highmere's south wall, fictionally established as the place where Lord Avermere trains new Avatars before sending them into Avermere proper. Always daylight, always fair weather, no hostile spawns. The garden is bounded by hedges; the only exit is a moongate at the north end.
...
| Items kept | All items the player crafts or picks up in the garden carry over to Highmere. |
...
| Animated arrow pointers ("Go this way") | Breaks "no hand-holding"; reduces Avermere to a theme-park ride. |
...
| XP rewards specifically for tutorial actions | Tutorials are free practice. Awarding XP encourages farming the tutorial; awarding none keeps it pure pedagogy. The four Highmere milestones (§3.4) and three garden sub-flags (§4.3) award **no XP and no Virtue deltas**. |
| Forced camera moves to highlight points of interest | Breaks the locked isometric view promise (Doc #2 §1, Doc #1 §2). |
| Dialogue from the game-engine narrator (not a diegetic NPC) | All exposition flows through diegetic NPCs (Erevan, the three tutors, Lord Avermere, etc.). The game never speaks in its own voice. |
...
> **Resolves Doc #7 §2's "Unlocked after completing the Highmere tutorial" requirement.**
...
| Highmere milestone sub-flags use the same channel | Milestone 1 writes its sub-flag from a `SetFlag` on the first non-trivial keyword response. Milestones 2–4 write from `Drag.complete`, `Examine.complete`, and `OpenShop.complete` events the dispatcher already emits for telemetry — the onboarding sub-flag is just another listener. |
...
| Concern | Treatment |
|---|---|
| Time-pressured controls | **Neither path requires any.** Highmere Town Opening can be played at any pace; combat is avoidable (the four milestones in §3.4 do not require combat). Tutorial Shard's spar with Toridan is opt-in; declining still counts toward Toridan's sub-flag (§4.3). |
...
Per Doc #11. Highmere Town Opening only — Tutorial Shard deferred to Phase 2.
...
| §3 Highmere Opening | Doc #11 (vertical slice opening sequence), Doc #15 §3 (companions), Doc #17 (dialogue verbs), Doc #13 §2 (verb registry) |
