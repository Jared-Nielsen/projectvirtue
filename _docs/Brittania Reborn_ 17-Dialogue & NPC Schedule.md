...
3. `[OPEN]` **Pathfinder algorithm.** §7 specifies "the same nav system used by `move_to`" but Doc #14 §5.11 also defers. A* with dynamic obstacle re-plan is the working assumption; navmesh authoring tooling (Doc #9) is parallel work.
