...
- **OAuth link:** players can link their Discord account to their Project Virtue account for in-game perks (a cosmetic ribbon, access to a community channel). Linking is optional and severable.
- **Guild bot:** a Project Virtue Discord bot exposes guild-level commands (`/who`, `/raidcall`, `/uptime`) for community management. The bot is a *client* of the Rust server's public API; it has no privileged access.
