...
- **Lotcheck** — Nintendo's certification program. Project Virtue is **not** targeting Switch in current scope (§1.4); Lotcheck addressed only briefly.
...
Project Virtue's policy (§17): every Must-flag is fixed; every Should-flag is fixed unless an explicit dispensation is filed and signed off by Console Platform Lead.
...
Steam Deck, Steam Big Picture, ROG Ally, and similar handheld-PC form factors are **not console certifications** but share input/UI surface with consoles. Project Virtue's gamepad UI mode (§4.1) is shared between consoles and Steam Deck on the UE5 client; Steam Deck Verified is a separate certification program owned by the PC platforms doc (TBD), not this one.
...
**Phase 3 success metric:** Project Virtue passes TRC and XR on first cert submission with ≤ 5 fixable Should-flag items and zero Must-flag items, launches simultaneously on PS5 and Xbox Series X|S, and a player buying premium currency on PS5 sees the balance reflected on Xbox within 60 seconds of cross-platform sign-in.
