Slide 1: Title  
ULTIMA VII: BRITANNIA REBORN  
An Official Licensed Remake Proposal \[Visual: Classic Ultima VII Britain town at dusk with subtle modern lighting glow\]  
Subtitle: A faithful isometric simulation RPG that becomes a living platform for generations.  
Presented to: Richard Garriott (Lord British)  
\[Your Studio Name\] | May 2026

---

 Slide 2: The Vision in One Sentence  
We are not remaking Ultima VII.  
We are reviving it as the persistent, community-shaped Britannia you always envisioned. \[Visual: Split image – left: 1992 original screenshot | right: modern prototype mockup of same scene\]

---

 Slide 3: Agenda

1. Why This Matters  
2. Creative Foundation (Documents 1–5)  
3. Living World Systems (Documents 6–8)  
4. Art & Audio Fidelity  
5. The Prototype We Will Build  
6. Partnership & Next Steps

---

Slide 4: What We Will Never Change

* Locked isometric perspective  
* Full-screen simulation (every object matters)  
* Real-time world with NPC schedules  
* The Eight Virtues as moral core  
* No platformer mechanics. Ever.

\[Visual: Green checkmarks \+ original Ultima VII screenshots\]

---

 Slide 5: Modern Inspirations – Applied with Respect

* Roblox → In-game creation tools & UGC marketplace  
* Unreal Engine → Deep modding & extensibility  
* Star Citizen → Persistent shared world  
* No Man’s Sky → Guided procedural expansion

All in service of your original vision.

---

 Slide 6: Core Game Pillars (Document \#2)

* Simulation First  
* Virtue-Driven Morality  
* Open-Ended Freedom  
* Community Creation  
* Persistent Living World

\[Visual: Simple 5-pillar icon grid\]

---

 Slide 7: The World Bible – Britannia Eternal (Document \#3)  
Faithful 1992 map \+ controlled expansion.  
Canon rules protect the Eight Virtues and your legacy while allowing player stories. \[Visual: Classic Britannia map with subtle procedural frontier glow\]

---

 Slide 8: Simulation – The Heart of Ultima (Document \#4)  
Every object is physics-enabled, combinable, and reactive.  
Fire spreads. NPCs follow real schedules. The world feels alive. \[Visual: Before/after – static barrel vs. player stacking \+ igniting it\]

---

 Slide 9: Virtues Engine (Document \#5)  
Independent 0–100 tracking for each Virtue.  
Every action has visible, permanent consequences.  
Redemption is always possible. \[Visual: Eight Virtues wheel with example consequence arrows\]

---

 Slide 10: Persistent Multiplayer (Document \#6)  
Shared Britannia where friends (and thousands of Avatars) coexist.  
Seamless co-op \+ opt-in social systems. \[Visual: Multiple Avatars in Britain marketplace\]

---

 Slide 11: UGC & Creation Tools (Document \#7)  
In-game editor so any player can build quests, dungeons, and stories.  
Moderated marketplace with creator revenue share. \[Visual: Screenshot mockup of the simple drag-and-drop editor\]

---

 Slide 12: Engine & Stack (Document \#41)  
UE5 production client (desktop + PS5 + Xbox) + TS / PixiJS web thin-client + Rust authoritative server.  
The UE5 client is a "dumb view" — Rust owns simulation, rules, and persistence. Protobuf on the wire, codegen for Rust / C++ / TS.  
\[Visual: Three-box diagram — UE5 client and Web client both pointing at a single Rust server\]  

