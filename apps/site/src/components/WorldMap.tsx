// Interactive Sosaria map. The base image is /worldmap.svg (lightweight
// rendition of the parchment-style art in /_conceptart/Website/WorldMap.png).
// SVG <polygon> overlays make each region clickable; a side panel shows the
// active region's lore.
//
...
export const WorldMap: Component = () => {
  ...
  const zoomBy = (factor: number, originX = 0.5, originY = 0.5): void => {
    ...
    // Adjust translation so the zoom origin stays visually fixed.
  ...
  // Touch pinch — Pointer Events API gives us two simultaneous pointers.
  ...
  onMount(() => {
    if (isServer || !containerEl) return;
    containerEl.addEventListener('wheel', onWheel, { passive: false });
    onCleanup(() => containerEl?.removeEventListener('wheel', onWheel));
  });

  // Sync ARIA after active changes for SR users.
  let panelEl: HTMLDivElement | undefined;
  createEffect(() => {
    const r = active();
    if (panelEl && r && !isServer) {
        ...
        >
          ...
          >
            ...
            <g transform={transform()}>
              {/* Stylized parchment base. The full painted map ships at
                  /worldmap.png and can be swapped in via <image href> when
                  the asset pipeline produces the optimized variants. */}
              <defs>
